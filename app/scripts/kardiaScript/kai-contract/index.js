...
module.exports = KaiContract

function KaiContract (query) {
  return function contractFactory (contractABI, contractBytecode, contractDefaultTxObject) {
    // validate params
    if (!Array.isArray(contractABI)) { throw new Error(`[kai-contract] Contract ABI must be type Array, got type ${typeof contractABI}`) }
    if (typeof contractBytecode !== 'undefined' && typeof contractBytecode !== 'string') { throw new Error(`[kai-contract] Contract bytecode must be type String, got type ${typeof contractBytecode}`) }
    if (typeof contractDefaultTxObject !== 'undefined' && typeof contractDefaultTxObject !== 'object') { throw new Error(`[kai-contract] Contract default tx object must be type Object, got type ${typeof contractABI}`) }
