...
function Contract (opts = {}) {
  ...
  async function performCall ({ methodObject, methodArgs }) {
    ...
    return new Promise((resolve, reject) => {
      self.query[queryMethod](methodTxObject, (err, queryResult) => {
        if (err) {
          reject(err)
        }
        if (queryMethod === 'call') {
          ...
          try {
            ...
            const decodingError = new Error(`[kai-contract] while formatting incoming raw call data ${JSON.stringify(queryResult)} ${decodeFormattingError}`)
